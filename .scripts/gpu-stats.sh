...
if [ -z "$STATS_GPU_TOTAL_MEM" ]; then
    STATS_GPU_TOTAL_MEM="$(cat /sys/class/drm/card0/device/mem_info_vram_total)"
    export STATS_GPU_TOTAL_MEM="$(echo "scale=2; ${STATS_GPU_TOTAL_MEM}/${GIGABYTE}" | bc -l)"
fi

STATS_GPU_UTILIZATION="$(cat /sys/class/drm/card0/device/gpu_busy_percent)"
STATS_GPU_MEM="$(cat /sys/class/drm/card0/device/mem_info_vram_used)"
